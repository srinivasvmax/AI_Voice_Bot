...
def language_to_sarvam_language(language: Language) -> str:
    ...
    # Only supporting Telugu, Hindi, and English
    LANGUAGE_MAP = {
...
class SarvamSTTService(STTService):
    ...
    def _is_saarika(self) -> bool:
        """Check if using Saarika (STT) model family."""
        return "saarika" in (self.model_name or "").lower()
    
    def _is_saaras(self) -> bool:
        """Check if using Saaras (STT-Translate) model family."""
        return "saaras" in (self.model_name or "").lower()
    
    def _is_saarika_model(self, model: str) -> bool:
        """Check if model string is Saarika (STT) family."""
        return "saarika" in model.lower()
    
    def _is_saaras_model(self, model: str) -> bool:
        """Check if model string is Saaras (STT-Translate) family."""
        return "saaras" in model.lower()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Store model name first for helper methods
        self._model_name = model
        
        # Validate that saaras models don't accept language parameter
        if self._is_saaras_model(model):
        ...
        if self._is_saarika_model(model):
        ...
        if not hasattr(self, '_FrameProcessor__process_queue'):
            ...
            logger.debug("🔧 [STT] Applied _FrameProcessor__process_queue workaround")
    ...
    async def run_stt(self, audio: bytes):
        ...
        Yields:
            Frame: ErrorFrame if connection issues, None otherwise (transcription results come via WebSocket callbacks).
        """
        if not self._socket_client:
            yield ErrorFrame(error="Sarvam STT socket not connected")
            return
        ...
        try:
            ...
            # Disable audio chunk logging to prevent log flooding
            # Only log audio chunks in trace mode
            # logger.debug(f"🎤 [STT] Sending audio: {len(audio)} bytes, encoding={encoding}, sample_rate={self.sample_rate}")
    ...
    async def _connect(self):
        ...
        try:
            ...
            if self._is_saarika():
                ...
                if not self._language_string:
                    raise ValueError(f"Language code is required for saarika model but not provided")
                
                connect_kwargs["language_code"] = self._language_string
                logger.info(f"🎤 [STT] Connecting with language: {self._language_string} (from {self._language_code})")
            ...
            if self._prompt is not None and self._is_saaras():
    ...
    async def _disconnect(self):
        ...
        if self._websocket_context and self._socket_client:
            try:
                ...
                await self.push_error(error_msg=f"Error closing WebSocket connection: {e}", exception=e)
            finally:
    ...
    async def _handle_message(self, message):
        ...
        try:
            if message.type == "events":
                ...
                # For saarika models, enforce the configured language only
                if self._is_saarika():
                    # Use the configured language, not the detected one
                    language = self._language_code if self._language_code else Language.HI_IN
                    
                    # Validate that detected language matches expected (with warning if different)
                    if language_code and language_code != self._language_string:
                        logger.warning(f"🎤 [STT] Language mismatch: expected {self._language_string}, detected {language_code}. Using configured language.")
                else:
                    # For translate models, use detected language
                    if language_code:
                ...
                # Log all transcription attempts (including empty ones)
                logger.info(f"🎤 [STT] Transcription response: transcript='{transcript}', language={language_code}, duration={message.data.metrics.audio_duration}s")
                
                if transcript and transcript.strip():
                    logger.info(f"🎤 [STT] Valid transcription received: '{transcript}' (detected: {language_code}, expected: {self._language_string})")
                    
                    # For saarika models, validate language match if strict mode is enabled
                    from app.config import settings
                    if (self._is_saarika() and 
                        self._language_string and 
                        settings.STT_STRICT_LANGUAGE_MODE):
                        if language_code and language_code != self._language_string:
                            logger.warning(f"🎤 [STT] Rejecting transcription due to language mismatch: expected {self._language_string}, got {language_code}")
                    ...
                    logger.info(f"🎤 [STT] Pushing transcription frame to pipeline: '{transcript}' (language: {language})")
                    logger.info(f"🎤 [STT] TranscriptionFrame details: text='{transcription_frame.text}', user_id={transcription_frame.user_id}, timestamp={transcription_frame.timestamp}")
                    await self.push_frame(transcription_frame)
                    logger.info(f"🎤 [STT] TranscriptionFrame pushed successfully")
                    
                    await self.stop_processing_metrics()
                else:
                    logger.warning(f"🎤 [STT] Empty transcription received - audio may be unclear or model issue (duration: {message.data.metrics.audio_duration}s)")

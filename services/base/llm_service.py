...
class FunctionCallResultCallback(Protocol):
    ...
    async def __call__(self, result: Any, *, properties: Optional[FunctionCallResultProperties] = None) -> None:
...
class LLMService(AIService):
    ...
    async def run_function_calls(self, function_calls: Sequence[FunctionCallFromLLM]):
        ...
        for function_call in function_calls:
            if function_call.function_name in self._functions.keys():
                ...
                logger.warning(f"{self} is calling '{function_call.function_name}', but it's not registered.")
                continue
    ...
    async def _call_start_function(self, context: OpenAILLMContext | LLMContext, function_name: str):
    ...
    async def _run_function_call(self, runner_item: FunctionCallRunnerItem):
        ...
        logger.debug(f"{self} Calling function [{runner_item.function_name}:{runner_item.tool_call_id}] with arguments {runner_item.arguments}")
